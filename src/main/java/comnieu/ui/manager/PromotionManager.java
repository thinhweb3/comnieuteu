...
public class PromotionManager extends javax.swing.JFrame {
    ...
    public void fillToTable() {
        ...
        for (Promotion p : items) {
            Object[] row = {
                p.getId(),
                p.getName(),
                XDate.format(p.getStartDate(), XDate.PATTERN_SHORT),
                XDate.format(p.getEndDate(), XDate.PATTERN_SHORT),
                (p.getDiscountRate()) + "%",
            };
            model.addRow(row);
        }
    ...
    txtStartDate.setText(XDate.format(entity.getStartDate(), XDate.PATTERN_SHORT));
    txtEndDate.setText(XDate.format(entity.getEndDate(), XDate.PATTERN_SHORT));
    ...
    try {
        ...
        if (!idText.isEmpty()) {
            entity.setId(Integer.getInteger(idText));
        }

        entity.setName(txtName.getText().trim());

        entity.setStartDate(XDate.parse(txtStartDate.getText().trim(), XDate.PATTERN_SHORT));
        entity.setEndDate(XDate.parse(txtEndDate.getText().trim(), XDate.PATTERN_SHORT));

        String discountText = txtDiscount.getText().trim().replace("%", "");
        double discount = Double.parseDouble(discountText) / 100;
        entity.setDiscountRate((float) 0.1);
    ...
    tblPromotion.clearSelection(); // Bỏ chọn bảng nếu có
}

    public void deleteCheckedItems() {
        if (XDialog.confirm("Bạn thực sự muốn xóa các mục chọn?")) {
            for (int i = 0; i < tblPromotion.getRowCount(); i++) {
                if ((Boolean) tblPromotion.getValueAt(i, 4)) {
